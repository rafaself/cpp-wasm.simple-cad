...
PickResult CadEngine::pickSelectionHandle(float x, float y, float tolerance) const noexcept {
    ...
    if (meta.valid && meta.selectionCount > 0 && selection.size() == meta.selectionCount) {
        ...
        if (meta.hasSideHandles) {
            considerHandle(PickSubTarget::ResizeHandle, 4, meta.northX, meta.northY); // N
            considerHandle(PickSubTarget::ResizeHandle, 5, meta.eastX, meta.eastY);   // E
            considerHandle(PickSubTarget::ResizeHandle, 6, meta.southX, meta.southY); // S
            considerHandle(PickSubTarget::ResizeHandle, 7, meta.westX, meta.westY);   // W
        }
...
PickResult CadEngine::pickSideHandle(float x, float y, float tolerance) const noexcept {
    ...
    if (kind == EntityKind::Line || kind == EntityKind::Arrow || kind == EntityKind::Polyline || kind == EntityKind::Text) {
        return res;
    }
