...
function calculateDiscoverabilityScore(analytics: RibbonAnalyticsReport): number {
  ...
  const tooltipScore =
    Math.min(analytics.eventCounts.tooltip / analytics.eventCounts.click, 1) * 50; // Max 50 points
...
export async function generateBaselineReport(): Promise<BaselineReport> {
  ...
  const shortcutUsageRate =
    totalActions > 0 ? analyticsReport.eventCounts.shortcut / totalActions : 0;
...
export function formatBaselineReport(report: BaselineReport): string {
  ...
  lines.push(
    `Viewport: ${report.metadata.viewportSize.width}×${report.metadata.viewportSize.height}`,
  );
  lines.push('');
  ...
  lines.push(
    `  Range: ${report.alignment.groupBodyHeights.min.toFixed(1)}px - ${report.alignment.groupBodyHeights.max.toFixed(1)}px`,
  );
 ...
 */
export function downloadBaselineReport(
