...
export function useRibbonTracking(tabId: string, groupId?: string): RibbonTrackingHook {
  ...
  const trackHover = useCallback((itemId: string, duration: number) => {
    ribbonAnalytics.trackHover(itemId, duration, contextRef.current);
  }, []);

  const trackShortcut = useCallback((itemId: string, shortcutKey: string) => {
  ...
  const trackMisclick = useCallback((itemId: string, undoDelay: number) => {
...
export function useRibbonMisclickDetector() {
  ...
  const checkForMisclick = useCallback(() => {
    if (lastClickRef.current) {
      ...
      if (undoDelay < 3000) {
        ribbonAnalytics.trackMisclick(lastClickRef.current.itemId, undoDelay, {
