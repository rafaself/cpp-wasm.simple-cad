...
class RibbonAnalytics {
  ...
  getEvents(filter?: Partial<{ type: RibbonEventType; itemId: string; sessionId: string }>): RibbonEvent[] {
  ...
  getReport(): RibbonAnalyticsReport {
    ...
    const shortcutVsClick = Object.keys(shortcutUsage).map(itemId => ({
      ...
      ratio: shortcutUsage[itemId].count / ((clickCounts[itemId] || 0) + shortcutUsage[itemId].count)
    }))
