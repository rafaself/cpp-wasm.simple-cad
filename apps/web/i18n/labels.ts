export const LABELS = {
  common: {
    ok: 'OK',
    cancel: 'Cancelar',
    confirm: 'Confirmar',
    close: 'Fechar',
    edit: 'Editar',
    delete: 'Excluir',
    loading: 'Carregando...',
    none: 'Nenhum',
    unknown: 'Desconhecido',
    clickToEdit: 'Clique para editar',
    errorInvalidFile: 'Arquivo inválido ou corrompido.',
    errorNoSnapshot: 'Arquivo sem snapshot ESNP. Abra apenas documentos vNext.',
    fullScreenEnter: 'Entrar no modo Tela Cheia',
    fullScreenExit: 'Sair do modo Tela Cheia',
    fullScreenToggle: 'Alternar Tela Cheia',
    fullScreenMessageEnter: 'Para alternar o modo tela cheia, pressione:',
    fullScreenMessageExit: 'Você está no modo tela cheia. Para sair, pressione:',
    fullScreenInstruction: 'Use {shortcut} para entrar ou sair do modo tela cheia.',
    toggleTheme: 'Alternar Tema',
    selectPlaceholder: 'Selecione...',
    openPresets: 'Abrir predefinições',
  },
  colorPicker: {
    title: 'Seletor de Cor',
    eyedropper: 'Conta-gotas',
  },
  menu: {
    file: 'Arquivo',
    newFile: 'Novo Arquivo',
    openFile: 'Abrir Arquivo',
    saveFile: 'Salvar',
    undo: 'Desfazer',
    redo: 'Refazer',
    settings: 'Configurações',
  },
  tools: {
    select: 'Seleção',
    pan: 'Pan (Arrastar)',
    line: 'Linha',
    rect: 'Retângulo',
    circle: 'Círculo',
    polygon: 'Polígono',
    polyline: 'Polilinha',
    arrow: 'Seta',
    text: 'Texto',
    eraser: 'Borracha',
    polygonSides: 'Lados do Polígono',
    polygonSidesHint: 'Enter para confirmar • Esc para cancelar',
    polygonSidesSuffix: 'lados',
  },
  statusbar: {
    zoomIn: 'Aumentar Zoom',
    zoomOut: 'Diminuir Zoom',
    snapOptions: 'Opções de Snap',
    snapToGrid: 'Snap à Grade',
    snapToObject: 'Snap ao Objeto',
    coordinates: 'Coordenadas',
    commandPlaceholder: 'Comando...',
    commandTooltip:
      'Digite um comando (Ex: L para Linha, R para Retângulo). Pressione Enter para executar. Use Ctrl+K para focar.',
    commandUnknown: 'Comando desconhecido: {command}',
    commandReady: 'Pronto',
    commandReadyHint: 'Digite um comando',
    commandPressEnter: '[Enter]',
    commandInputLabel: 'Entrada de comando',
    commandExecute: 'Executar comando (Enter)',
    commandExecuteLabel: 'Executar comando',
    commandPalette: 'Abrir paleta de comandos',
    commandHelp: 'Ajuda de Comandos (HELP)',
    commandHelpLabel: 'Ajuda de Comandos',
    commandHelpDialogTitle: 'Comandos Disponíveis',
  },
  sidebar: {
    layers: 'Camadas',
    newLayer: 'Nova Camada',
    showLayer: 'Mostrar',
    hideLayer: 'Ocultar',
    lockLayer: 'Bloquear',
    unlockLayer: 'Desbloquear',
    deleteLayer: 'Excluir Camada',
    layerName: 'Nome da Camada',
  },
  settings: {
    general: 'Geral',
    snapping: 'Snap / Precisão',
    canvas: 'Canvas / Grade',
    shortcuts: 'Atalhos',
    snapActive: 'Snap Ativo',
    endpoints: 'Extremidades (Endpoints)',
    midpoints: 'Pontos Médios (Midpoints)',
    centers: 'Centros',
    grid: 'Grade (Grid)',
    nearest: 'Mais Próximo (Nearest)',
    gridSize: 'Tamanho da Grade',
    showGrid: 'Mostrar Grade',
    showAxis: 'Eixos Centrais',
    project: 'Projeto',
    projectTitle: 'Título do Projeto',
  },
  text: {
    bold: 'Negrito',
    italic: 'Itálico',
    underline: 'Sublinhado',
    strike: 'Tachado',
    alignLeft: 'Alinhar à Esquerda',
    alignCenter: 'Centralizar',
    alignRight: 'Alinhar à Direita',
    fontSize: 'Tamanho da Fonte',
    fontFamily: 'Fonte',
    color: 'Cor',
    editContent: 'Clique para editar',
    mixed: '—',
  },
  colors: {
    group: 'Cores',
    stroke: 'Traço',
    fill: 'Preenchimento',
    noFill: 'Sem preenchimento',
    inheritedTooltip: 'Cor herdada da camada "{nome}"',
    overrideTooltip: 'Cor personalizada do elemento',
    noneTooltip: 'Sem preenchimento',
    mixedTooltip: 'Múltiplos valores',
    disabledHint: 'Selecione um elemento ou ative uma ferramenta de desenho para editar cores',
    restoreTooltip: 'Restaurar cor da camada "{layer}"',
  },
  ribbon: {
    descriptions: {
      'new-file': 'Cria um novo arquivo de projeto.',
      'open-file': 'Abre um arquivo existente.',
      'save-file': 'Salva o projeto atual.',
      'export-json': 'Exporta o projeto em JSON.',
      'export-project': 'Exporta o pacote completo do projeto.',
      select: 'Seleciona e modifica elementos.',
      line: 'Desenha uma linha.',
      polyline: 'Desenha uma polilinha com múltiplos segmentos.',
      arrow: 'Desenha uma seta.',
      rect: 'Desenha um retângulo.',
      circle: 'Desenha um círculo.',
      polygon: 'Desenha um polígono.',
      undo: 'Desfaz a última ação.',
      redo: 'Refaz a última ação desfeita.',
      text: 'Insere texto no desenho.',
      pan: 'Move a vista sem alterar o desenho.',
      'zoom-to-fit': 'Ajusta a vista para todo o conteúdo.',
      grid: 'Mostra ou oculta a grade.',
      measure: 'Mede distâncias no desenho.',
    },
  },
  disciplines: {
    architecture: 'Arquitetura',
    electrical: 'Elétrica',
    shortArchitecture: 'Arq',
    shortElectrical: 'Elé',
  },
};
