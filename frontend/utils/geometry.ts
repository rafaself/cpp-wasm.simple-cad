...
export const getWrappedLines = (text: string, maxWidth: number, fontSize: number): string[] => {
  ...
  // Cache key: content-width-size
  const cacheKey = `${text}-${maxWidth.toFixed(2)}-${fontSize}`;
  if (wrappedLinesCache.has(cacheKey)) {
      return wrappedLinesCache.get(cacheKey)!;
  }

  // Basic cache size management
  if (wrappedLinesCache.size > 1000) {
      // Clear oldest entries (approximation by clearing half or all)
      // Map preserves insertion order, so we can delete the first key.
      const firstKey = wrappedLinesCache.keys().next().value;
      if (firstKey) wrappedLinesCache.delete(firstKey);
  }
...
export const getTextDimensions = (shape: Shape) => {
  ...
  // When shape.width is defined, use it directly for wrapping calculations
  // When not defined, we calculate width based purely on text content (no padding)
  const baseWidth = shape.width ? Math.max(shape.width, 1) : undefined;

  const wrapped = baseWidth
    ? getWrappedLines(rawText, baseWidth, fontSize)
    : rawText.split('\n');

  // Calculate text width based on actual text content - no padding in bounding box
  ...
  // Use explicit width if set, otherwise use estimated width (no padding added)
  const totalWidth = shape.width ?? estimatedWidth;
  const totalHeight = shape.height ?? estimatedHeight;
