...
export class EngineRuntime {
  ...
  public setEntityOverride(
    ...
    if (!this.#engine.setEntityOverride || !this.#engine.allocBytes || !this.#engine.freeBytes) {
      return;
    }
    ...
    try {
      ...
      u32.set(ids);
      this.#engine.setEntityOverride(ptr, ids.length, isStroke, r, g, b, a);
    } finally {
  ...
  public setFillEnabled(ids: EntityId[], enabled: boolean): void {
    if (!this.#engine.setFillEnabled || !this.#engine.allocBytes || !this.#engine.freeBytes) {
      return;
    }
    const ptr = this.#engine.allocBytes(ids.length * 4);
    try {
      const u32 = new Uint32Array(this.module.HEAPU8.buffer, ptr, ids.length);
      u32.set(ids);
      this.#engine.setFillEnabled(ptr, ids.length, enabled);
    } finally {
      this.#engine.freeBytes(ptr);
    }
  }
  ...
  public clearEntityOverride(ids: EntityId[], isStroke: boolean): void {
    if (!this.#engine.clearEntityOverride || !this.#engine.allocBytes || !this.#engine.freeBytes) {
      return;
    }
    const ptr = this.#engine.allocBytes(ids.length * 4);
    try {
      const u32 = new Uint32Array(this.module.HEAPU8.buffer, ptr, ids.length);
      u32.set(ids);
      this.#engine.clearEntityOverride(ptr, ids.length, isStroke);
    } finally {
      this.#engine.freeBytes(ptr);
    }
  }
