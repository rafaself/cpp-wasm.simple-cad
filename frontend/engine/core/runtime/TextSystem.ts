...
export class TextSystem {
  ...
  public getTextSelectionRects(textId: number, start: number, end: number) {
    return this.engine.getTextSelectionRects?.(textId, start, end) ?? { size: () => 0, get: () => null as any, delete: () => undefined };
  }
  ...
  public setTextPosition(textId: number, x: number, y: number, boxMode: number, constraintWidth: number): boolean {
