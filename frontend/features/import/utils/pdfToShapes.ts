...
import { Matrix, IDENTITY_MATRIX, multiplyMatrix, applyMatrix, formatColor, isNearWhiteHex } from './pdfMatrixUtils';

const isDevEnv = typeof process !== 'undefined' && process.env?.NODE_ENV !== 'production';
const CTM_TRANSLATION_WARN = 1_000_000, CTM_SCALE_WARN = 10_000;

interface PdfImportColorOptions { colorScheme?: DxfColorScheme; customColor?: string; }
interface GraphicsState { ctm: Matrix; strokeColor: string; fillColor: string; lineWidth: number; }
      ...
      shapes.forEach(s => {
          if (s.type === 'line' || s.type === 'polyline') {
              s.points?.forEach(p => {
               ...
               else if ('height' in s && typeof (s as any).height === 'number') shapeHeight = (s as any).height;
