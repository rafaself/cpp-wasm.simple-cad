...
const NumberSpinner: React.FC<NumberSpinnerProps> = ({ 
    ...
    return (
        <div className={`flex items-center bg-slate-800/60 border ${isFocused ? 'border-blue-500/50 ring-1 ring-blue-500/20' : 'border-slate-700/50'} rounded ${className || 'w-[60px]'} relative overflow-hidden transition-all group`}>
            <input
                type="text"
                value={tempValue}
            ...
            {suffix && !isFocused && <span className="absolute right-5 pointer-events-none text-[10px] text-slate-300 pt-0.5">{suffix}</span>}
