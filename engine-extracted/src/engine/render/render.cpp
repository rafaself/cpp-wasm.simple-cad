...
void rebuildRenderBuffers(
    ...
    // Stable 2D z-order:
    // 1) elevationZ ascending (back to front so higher Z draws last),
    // 2) explicit draw-order rank,
    // 3) entity id for deterministic ties.
    std::unordered_map<std::uint32_t, std::uint32_t> drawRank;
    drawRank.reserve(ordered.size());
    for (std::uint32_t i = 0; i < ordered.size(); ++i) {
        drawRank[ordered[i]] = i;
    }

    auto elevationFor = [&](std::uint32_t id) -> float {
        const auto it = entities.find(id);
        if (it == entities.end()) return 0.0f;
        const EntityRef ref = it->second;
        switch (ref.kind) {
            case EntityKind::Rect:
                if (ref.index < rects.size() && std::isfinite(rects[ref.index].elevationZ)) return rects[ref.index].elevationZ;
                return 0.0f;
            case EntityKind::Line:
                if (ref.index < lines.size() && std::isfinite(lines[ref.index].elevationZ)) return lines[ref.index].elevationZ;
                return 0.0f;
            case EntityKind::Polyline:
                if (ref.index < polylines.size() && std::isfinite(polylines[ref.index].elevationZ)) return polylines[ref.index].elevationZ;
                return 0.0f;
            case EntityKind::Circle:
                if (ref.index < circles.size() && std::isfinite(circles[ref.index].elevationZ)) return circles[ref.index].elevationZ;
                return 0.0f;
            case EntityKind::Polygon:
                if (ref.index < polygons.size() && std::isfinite(polygons[ref.index].elevationZ)) return polygons[ref.index].elevationZ;
                return 0.0f;
            case EntityKind::Arrow:
                if (ref.index < arrows.size() && std::isfinite(arrows[ref.index].elevationZ)) return arrows[ref.index].elevationZ;
                return 0.0f;
            default:
                return 0.0f;
        }
    ...
    std::stable_sort(ordered.begin(), ordered.end(), [&](std::uint32_t a, std::uint32_t b) {
        const float za = elevationFor(a);
        const float zb = elevationFor(b);
        if (za != zb) return za < zb;
        const std::uint32_t ra = drawRank.count(a) ? drawRank[a] : 0;
        const std::uint32_t rb = drawRank.count(b) ? drawRank[b] : 0;
